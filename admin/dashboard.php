...
// GETTING LOGGED IN ADMIN DETAILS
...
$all_users_query = mysqli_query($conn, $all_users_sql);
        ...
        <section id="cart" class="section-p1">
            ...
            <table width="100%">
                ...
                <tbody>
                    ...
                    <?php while($users = mysqli_fetch_assoc($all_users_query)):?>

                    

                    <tr>
                        <td> <a href="<?= ROOT_URL ?>admin/delete-user.php?id=<?= $users['id'] ?>"><i class="far fa-times-circle"></i></a> </td>
                        <td><?= $users['id'] ?></td>
                        <td><?= $users['firstname'] ?></td>
                        <td><?= $users['lastname'] ?></td>
                        <td><?= $users['email'] ?></td>
                        <td><?= $users['phone'] ?></td>
                        <td><a href="<?= ROOT_URL ?>admin/edit-user.php?id=<?= $users['id'] ?>"><i class="fa-solid fa-screwdriver-wrench"></i></a></td>
                    </tr>
                    <?php endwhile; ?>
                    <?php else :?>
                        <p class="bg-danger p-2">You currently have no registered users</p>
                    <?php endif; ?>
